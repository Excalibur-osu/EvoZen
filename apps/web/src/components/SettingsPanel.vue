...
function handleFileImport(e: Event) {
  ...
  reader.onload = async (re) => {
    ...
    if (ok) {
      ...
      if (success) game.toggleSettings()
      else importError.value = '导入失败，存档文件损坏或无效。'
    }
      ...
      <div class="settings-modal-overlay" v-if="game.isSettingsOpen" @click.self="game.toggleSettings()">
        <div class="settings-modal">
          <div class="modal-header">
            <h3 class="panel-title"><AppIcon name="settings" /> 系统设置</h3>
            <IconButton icon="close" label="关闭设置" tone="danger" @click="game.toggleSettings()" />
          </div>
        ...
        <div class="setting-group">
          ...
          <div class="action-row">
            <button class="btn" @click="handleExportFile">
              <AppIcon name="fileDown" />
              <span>导出存档 (.json)</span>
            </button>
            
            <input class="hidden-file-input" type="file" ref="fileInputRef" accept=".json" @change="handleFileImport" />
            <button class="btn" @click="fileInputRef?.click()">
              <AppIcon name="fileUp" />
              <span>导入存档</span>
            </button>
          </div>
...
.settings-modal {
  ...
  max-width: 600px;
  max-height: 90vh;
  box-shadow: 0 12px 32px color-mix(in srgb, var(--bg-primary) 75%, transparent);
  display: flex;
...
.setting-group {
  ...
  box-shadow: 0 4px 12px color-mix(in srgb, var(--bg-primary) 45%, transparent);
}
.setting-group h4 {
  margin: 0 0 8px;
  color: var(--text-primary);

...
export const useGameStore = defineStore('game', () => {
    ...
    function applyBuildingEffects() {
        ...
        // --- 毛皮显示（有猎人工作时自动显示）---
        const hunterWorkers = s.civic['hunter']?.workers ?? 0;
        if (hunterWorkers > 0) {
        ...
        for (const job of BASE_JOBS) {
            if (job.id === 'unemployed' || job.id === 'hunter')
                continue;
            if (!job.requiredTech)
                continue;
            let unlocked = true;
        ...
        const hasAnyJob = BASE_JOBS.some(j => {
            if (j.id === 'unemployed' || j.id === 'hunter')
                return false;
            return s.civic[j.id]?.display;
        });
        if (hasAnyJob) {
    ...
    function getStructCount(id) {
        return state.value.city[id]?.count ?? 0;
    }
    function setJobMax(jobId, max) {
        const job = state.value.civic[jobId];
        if (job)
            job.max = max;
    }
    /** 人口增长：有食物且有空房间时缓慢增长 */
    function handlePopGrowth() {
        ...
        if (pop.amount < pop.max && food.amount > food.max * 0.1) {
            ...
            if (Math.floor(pop.amount) > Math.floor(pop.amount - 0.005)) {
                ...
                // 新市民默认为失业
                const unemployed = s.civic['unemployed'];
                if (unemployed)
                    unemployed.workers++;
            }
        }
    ...
    /** 添加消息 */
    function addMessage(text, type = 'info', category = 'progress') {
    ...
    /** 获取建筑费用 */
    function getBuildCost(structureId) {
        const def = BASIC_STRUCTURES.find(s => s.id === structureId);
        if (!def)
            return {};
        const count = getStructCount(structureId);
    ...
    function build(structureId) {
        ...
        if (!state.value.city[structureId]) {
            state.value.city[structureId] = { count: 0, on: 0 };
        }
        const building = state.value.city[structureId];
        building.count++;
    ...
    function isTechAvailable(techId) {
        ...
        if ((state.value.tech[grantKey] ?? 0) >= grantLvl)
            return false;
        for (const [reqKey, reqLvl] of Object.entries(def.reqs)) {
            if ((state.value.tech[reqKey] ?? 0) < reqLvl)
                return false;
        }
        return true;
    }
    function canAffordTech(techId) {
        ...
        for (const [resId, cost] of Object.entries(def.costs)) {
            if ((state.value.resource[resId]?.amount ?? 0) < cost)
                return false;
        }
        return true;
    }
    ...
    function assignWorker(jobId) {
        const job = state.value.civic[jobId];
        const unemployed = state.value.civic['unemployed'];
        if (!job || !unemployed || unemployed.workers <= 0)
            return;
        if (job.max >= 0 && job.workers >= job.max)
            return;
        job.workers++;
        unemployed.workers--;
    }
    function removeWorker(jobId) {
        const job = state.value.civic[jobId];
        const unemployed = state.value.civic['unemployed'];
        if (!job || !unemployed || job.workers <= 0)
            return;
        job.workers--;
        unemployed.workers++;
    }
    ...
    /** 开始文明（离开进化阶段） */
    function startCivilization(speciesId) {
        const speciesLabels = {
        ...
        // 设置初始猎人（1个人口全是猎人）
        const hunter = state.value.civic['hunter'];
        if (hunter) {
            hunter.workers = 1;
            hunter.display = true;
        }
    ...
    function gather(resourceId) {
        ...
        let amount = 1;
        // 石斧科技加成
        if (resourceId === 'Lumber' && (state.value.tech['axe'] ?? 0) >= 1)
            amount = 2;
        if (resourceId === 'Stone' && (state.value.tech['mining'] ?? 0) >= 1)
            amount = 2;

...
export function gameTick(state) {
    ...
    const structCount = (id) => state.city[id]?.count ?? 0;
    const workers = (id) => state.civic[id]?.workers ?? 0;
    const techLevel = (id) => state.tech[id] ?? 0;
    // ============================================================
    // 1. 食物
    // ============================================================
    ...
    let farmerBase = 0.82;
    if (techLevel('agriculture') >= 2)
        farmerBase += 0.36; // 灌溉 +40%
    if (techLevel('agriculture') >= 4)
        farmerBase += 0.12; // 磨坊科技
    // 磨坊建筑 +5%/座

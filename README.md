...
- 文明时代核心循环已经基本可玩。
- 太空内容已覆盖早期太空到深空全链路（近地轨道 → 月球 → 火星 → 地狱行星 → 恒星 → 气态巨行星 → 小行星带 → 矮行星）。
- 星际内容已恢复 Alpha 首段可玩链路（`alpha_mission → starport → mining_droid / habitat`）。
- 完整种族分支、Prestige / Reset、Portal / 终局内容仍未恢复。
